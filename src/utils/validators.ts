...
export const lavanderiaSchema = z.object({
  ...
  telefone: z.string().min(8).max(20).optional().or(z.literal("").transform(() => undefined)),
  horarioAbertura: z.string().optional().or(z.literal("").transform(() => undefined)),
  horarioFechamento: z.string().optional().or(z.literal("").transform(() => undefined)),
  descricao: z.string().optional().or(z.literal("").transform(() => undefined)),
  fotoUrl: z.string().url().optional().or(z.literal("").transform(() => undefined)),
...
export const maquinaSchema = z.object({
  ...
  status_maquina: z.enum(["DISPONIVEL", "EM_USO", "EM_MANUTENCAO"]).default("DISPONIVEL"),
